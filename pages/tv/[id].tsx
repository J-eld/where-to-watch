...
const TV_id: React.FC = ({}) => {
  ...
  useEffect(() => {
    if (id !== undefined) {
      axios
        .get(`/api/search/tv/${router.query.id}`)
        .then((res) => {
          const info = res.data;
          setTVInfo(info);
      ...
      <Container maxWidth="xl">
        ...
        {TVInfo.title && (
          <div className={styles.movieSearchBody}>
            <div className={styles.movieTitle}>{TVInfo && TVInfo.title}</div>
            <div className={styles.movieInfo}>
              <div className={styles.movieImage}>
                {TVInfo && (
                  <Image
                    src={`https://image.tmdb.org/t/p/w342${TVInfo.poster_path}`}
                    width={342}
                    height={513}
                  />
                )}
                  ...
                  <div className={styles.streamingPlatformsList}>
                    {getCountryBlocked ? (
                      ...
                      (streams[localCountry]?.length > 0 ? (
                        ...
                        <ErrorNotAvailableInCountry mediaType="tv" />
                      ) : (
                        <ErrorNotAvailable mediaType="tv" />
                      ))
                    )}
                  </div>
                </div>
