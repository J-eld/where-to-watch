...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const streamingProviders = await axios.get(
    `https://api.themoviedb.org/3/tv/${req.query.id}/watch/providers?api_key=${process.env.NEXT_PUBLIC_API_KEY}`
  );
  ...
  const show = await axios.get(
    `https://api.themoviedb.org/3/tv/${req.query.id}?api_key=${process.env.NEXT_PUBLIC_API_KEY}`
  );
